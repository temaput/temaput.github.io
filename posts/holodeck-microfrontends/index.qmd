...
```javascript
	...
	name: "host",
	remotes: {
		remote: `remote@${CLIENT_LOOKBOOK_APP_URL}/remoteEntry.js`,
	},
	shared: {
		react: {
		singleton: true,
...
Переменная CLIENT_LOOKBOOK_APP_URL содержит адрес веб-версии HoloProd. Ее мы передаем в скрипт через переменные окружения.

```javascript
const CLIENT_LOOKBOOK_APP_URL =
  process.env.CLIENT_LOOKBOOK_APP_URL || "http://localhost:3002";
  ```
...
![First attempt](ContainerComposition1.svg)
